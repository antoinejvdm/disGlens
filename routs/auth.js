...
authRouter.post("/api/reggister", async (req, res) => {
    try {
        ...
        const userCheck = await db.query('SELECT * FROM users WHERE email = $1 OR user_id = $2', [email, user_id]);
...
try {
    ...
    if (!email || !password) {
        return res.status(400).json({ error: 'email or password are mising!' });
      }

    // Check if the email exists
    const emailChek = await db.query('SELECT * FROM users WHERE email = $1', [email]);
    if (!(emailChek.rows.length > 0)) {
        return res.status(400).json({ error: 'Email dus not exist create account first' });
    }
